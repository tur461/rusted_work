    ...
    unsigned int *size_of_fmd_ptr)
{
  ...
  if (width > 2000 || height > 2000)
    return FJFX_FAIL_IMAGE_SIZE_NOT_SUP;
  if (dpi < 300 || dpi > 1024)
    return FJFX_FAIL_IMAGE_SIZE_NOT_SUP;
  if (width * 500 < 150 * dpi || width * 500 > 812 * dpi)
    return FJFX_FAIL_IMAGE_SIZE_NOT_SUP; // in range 0.3..1.62 in
  if (height * 500 < 150 * dpi || height * 500 > 1000 * dpi)
    return FJFX_FAIL_IMAGE_SIZE_NOT_SUP; // in range 0.3..2.0 in
  size_t size = size_of_fmd_ptr ? *size_of_fmd_ptr : FJFX_FMD_BUFFER_SIZE;
  if (size < FJFX_FMD_BUFFER_SIZE)
    return FJFX_FAIL_OUTPUT_BUFFER_IS_TOO_SMALL;
  FRFXLL_DATA_TYPE dt = 0;
  switch (output_format)
  {
  case FJFX_FMD_ANSI_378_2004:
    dt = FRFXLL_DT_ANSI_FEATURE_SET;
    break;
  case FJFX_FMD_ISO_19794_2_2005:
    dt = FRFXLL_DT_ISO_FEATURE_SET;
    break;
  default:
  ...
  if (size_of_fmd_ptr)
    *size_of_fmd_ptr = (unsigned int)size;
